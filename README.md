...
> **Live Kalshi prediction market algotrader powered by the Sentient GRID ROMA multi-agent framework — swap between Grok, Claude, or GPT-4o with a single env var.**
...
Sentient Market Reader connects to [Kalshi](https://kalshi.com)'s live KXBTC15M prediction markets — binary contracts that resolve YES/NO based on whether BTC's price is higher at the end of each 15-minute window — and runs a **Sentient GRID ROMA (Recursive Open Meta-Agent)** pipeline to autonomously analyze the market and decide whether to trade.
...
The entire LLM layer is **provider-agnostic** — one env var to switch between Grok, Claude, or GPT-4o.
...
|---|---|---|---|
| `fast` — Atomizer, Executors, Sentiment | `grok-3-mini` | `claude-haiku-4-5-20251001` | `gpt-4o-mini` |
| `smart` — Planner, Aggregator, Probability | `grok-3` | `claude-sonnet-4-6` | `gpt-4o` |
...
- **Provider-agnostic** — one env var to switch the entire pipeline between Grok, Claude, or GPT-4o
- **Live + Paper mode** — toggle between real Kalshi order placement and simulated paper trading
- **Real-time dashboard** — 3-column layout: market card + signal panel | BTC chart + ROMA pipeline visualizer | performance + trade log
- **ROMA loading animation** — staged pipeline visualizer (Atomize → Plan → Execute → Aggregate → Extract) with per-stage glow effects, flowing connector animations, elapsed timer, rotating status messages
...
- **Typeable contract quantity** — BUY YES/NO input with direct number entry (1–500 contracts)
...
- API key for at least one LLM provider: [xAI](https://console.x.ai) · [Anthropic](https://console.anthropic.com) · [OpenAI](https://platform.openai.com)
...
# ── LLM Provider ────────────────────────────────────────────────────
AI_PROVIDER=grok         # anthropic | grok | openai
...
Hit **Run Cycle** on the dashboard to trigger the first ROMA analysis. Subsequent cycles run every 5 minutes. Paper mode is the default — no real orders are placed unless you toggle Live Trading on the dashboard and confirm.
...
│
├── lib/
│   ├── llm-client.ts               # Unified LLM interface (Grok / Claude / OpenAI)
...
│
├── components/
│   ├── AgentPipeline.tsx           # ROMA pipeline visualizer + loading animation
│   ├── MarketCard.tsx              # Live market data + BUY YES/NO trade panel
...
│   ├── Header.tsx                  # Live/Paper toggle, cycle countdown, UTC clock
...
|---|---|---|
| `AI_PROVIDER` | Yes | `grok` \| `anthropic` \| `openai` |
